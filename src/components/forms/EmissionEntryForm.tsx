...
export function EmissionEntryForm({ projectId, defaultDate, onSaved, mode = 'standalone', action, apiSubmitUrl }: {
  projectId: string;
  defaultDate?: string;
  onSaved?: (res: EmissionEntrySaved) => void;
  mode?: 'standalone' | 'daily-log';
  // Optional server action for standalone use; daily-log flow may use a different action signature
  action?: (formData: FormData) => Promise<any> | void;
  // When provided, the form will submit via fetch to this API and call onSaved with the JSON
  apiSubmitUrl?: string;
}) {
  ...
  // Category display helper and memoized label
  function categoryLabelMap(key: string) {
    ...
    if (TR_MAP[k]) return TR_MAP[k];
    return k.replace(/_/g, ' ').replace(/(^|\s)([a-zçğıöşü])/g, (m, p1, p2) => p1 + p2.toUpperCase());
  }
  const categoryDisplay = useMemo(() => categoryLabelMap(category), [category]);

  // Initialize date
  useEffect(() => {
    setDate((d) => d || defaultDate || new Date().toISOString().slice(0,10));
  }, [defaultDate]);
  ...
  // Debounced preview calculation (mirrors existing EntryForm logic)
  useEffect(() => {
    let cancelled = false;
  ...
  async function onSubmitLocal(e: React.FormEvent<HTMLFormElement>) {
    if (apiSubmitUrl) {
      ...
      try {
        ...
        const res = await fetch(apiSubmitUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        const json = await res.json();
        if (!res.ok) throw new Error(json?.error || 'Kayıt başarısız');
        onSaved?.(json);
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('app-toast', { detail: { message: 'Kaydedildi', variant: 'success' } }));
        }
      } catch (err: any) {
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('app-toast', { detail: { message: err?.message || 'Hata oluştu', variant: 'error' } }));
        }
      }
      return;
    }
  ...
  <form action={apiSubmitUrl ? undefined : action} className="space-y-8" onSubmit={onSubmitLocal}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    {activity ? (<input type="hidden" name="type" value={type} />) : null}
                    <select name="type" value={type} onChange={(e) => setType(e.target.value as any)} disabled={!!activity} required={!activity} className="form-input disabled:bg-white/5 disabled:border-white/5 disabled:text-white/70 appearance-none pr-10 w-full">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    {activity ? (<input type="hidden" name="scope" value={scope} />) : null}
                    <select name="scope" value={scope} onChange={(e) => setScope(e.target.value as any)} disabled={!!activity} className="form-input disabled:bg-white/5 disabled:border-white/5 disabled:text-white/70 appearance-none pr-10 w-full">
                      <option value="">Seç</option>
                    ...
                    <p className="text-xs text-white/50 mt-1">Doğrudan: kendi faaliyetlerinizden; Satın Alınan Enerji: tüketilen elektrik/ısı; Diğer Dolaylı: tedarik/taşımacılık vb.</p>
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="form-label flex items-center gap-2">
                  ...
                  <div className="relative">
                    {activity ? (
                      <>
                        <input value={categoryDisplay} readOnly className="form-input pl-10 disabled:bg-white/5 disabled:border-white/5 disabled:text-white/70 w-full" disabled />
                        <input type="hidden" name="category" value={category} />
                      </>
                    ) : (
                      <input name="category" value={category} onChange={(e) => setCategory(e.target.value)} placeholder="örn. elektrik" className="form-input pl-10 w-full" />
                    )}
                    <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-white/50">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 12V8h-4V4h-4v4H8v4H4v4h4v4h4v-4h4v-4h4z"/></svg>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="px-6 pb-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 rounded-md bg-gradient-to-br from-leaf-500/20 to-ocean-500/20 border border-white/10">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-leaf-400"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  <div className="relative flex-grow">
                    <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-white/50">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 16l-4-4-4 4"/></svg>
                    </div>
                    <input name="amount" type="number" step="any" value={amount || ''} onChange={(e) => setAmount(Number(e.target.value))} required className="form-input pl-10 w-full" placeholder="0.00" />
                  </div>
                  <div className="w-32">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input name="date" type="date" value={date} onChange={(e) => setDate(e.target.value)} required className="form-input pl-10 w-full [&::-webkit-calendar-picker-indicator]:bg-white/20 [&::-webkit-calendar-picker-indicator]:rounded [&::-webkit-calendar-picker-indicator]:hover:bg-white/30 [&::-webkit-calendar-picker-indicator]:p-1" />
                </div>
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 h-full">
              ...
              <div className="relative">
                <textarea name="notes" value={notes} onChange={(e) => setNotes(e.target.value)} className="form-input min-h-[160px] w-full" placeholder="Ek bilgiler, konum, tedarikçi vb." />
              </div>
            </div>
            <div className={`rounded-xl ${!calcError ? 'bg-gradient-to-br from-leaf-500/10 to-ocean-500/10 border border-leaf-500/20' : 'bg-gradient-to-br from-red-500/10 to-red-800/10 border border-red-500/20'} p-5 flex flex-col h-full`}>
              <div className="flex items-center gap-2 mb-4">
                <div className={`p-2 rounded-md ${!calcError ? 'bg-leaf-500/20' : 'bg-red-500/20'} border border-white/10`}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={calcError ? 'text-red-400' : 'text-leaf-400'}><path d="M18 8h1a4 4 0 0 1 0 8h-1"></path><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"></path><line x1="6" y1="1" x2="6" y2="4"></line><line x1="10" y1="1" x2="10" y2="4"></line><line x1="14" y1="1" x2="14" y2="4"></line></svg>
                </div>
              ...
              <div className="flex-grow flex flex-col justify-center items-center py-8">
                {calculating ? (
                  <div className="flex flex-col items-center text-white/70">
                    <svg className="animate-spin h-10 w-10 mb-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
              ...
              <div className="text-xs text-white/60 text-center border-t border-white/10 pt-3 mt-2">Değerler, seçilen aktivite ve miktara göre otomatik hesaplanır</div>
            </div>
          </div>
          <div className="mt-8 flex justify-end items-center">
            <Button type="submit" className="bg-gradient-to-r from-leaf-600 to-ocean-600 hover:from-leaf-500 hover:to-ocean-500 px-6 py-2.5 text-white rounded-lg">
              {mode === 'daily-log' ? 'Günlüğe Ekle' : 'Kaydı Tamamla'}
            </Button>
          </div>
        </div>
        {/* Hidden activity id */}
        <input type="hidden" name="activity_id" value={activity || ''} />
        {/* Hidden category display for read-only UI (used only for diagnostics) */}
        <input type="hidden" name="__category_display" value={categoryDisplay} />
      </form>
    </div>
  );
}
